...
to setup
  clear-all
  if add-obstacle = true [setup-walls]
  ...
  ;;set output-file "world_data.csv"
  set output-file (word "./Results/results " date-and-time ".csv")
  file-open output-file
  file-print "tick, ID, population, minimum-separation, max-align-turn, max-cohere-turn, max-separate-turn, vision, pxcor, pycor, obstacle, obstacle-density, random-walls, nearest-neighbor, flockmates"
  file-close
...
to setup-walls
  ifelse random-walls = true [
    create-random-walls
  ] [ create-wall ]
end

to create-random-walls
  let min-xcor-edge -30  ;; Adjust as needed
  let max-xcor-edge 30   ;; Adjust as needed
  let min-ycor-edge -30  ;; Adjust as needed
  let max-ycor-edge 30   ;; Adjust as needed

  ask patches[
    if (pxcor >= min-xcor-edge and pxcor <= max-xcor-edge
        and pycor >= min-ycor-edge and pycor <= max-ycor-edge)
       and random-float 100 < obstacle-density
    [      set pcolor green    ] ]
end

to create-wall
  let min-xcor-edge -1  ;; Adjust as needed
  let max-xcor-edge 1   ;; Adjust as needed
  let min-ycor-edge -1  ;; Adjust as needed
  let max-ycor-edge 1   ;; Adjust as needed
...
to go
  ...
  ask turtles [ flock ]
  repeat 5 [ ask turtles with [is-obstacle = 0] [
  if [pcolor] of patch-ahead 1 != green [ fd 0.2 ]] display ]
  ;;export-world (word "./Results/results " date-and-time ".csv") ;; Colon characters in the time cause errors on Windows
  tick
end

to export-world-to-file
  file-open output-file
  foreach sort turtles [ t ->
    ask t [
      file-print (word ticks "," self
        "," population
        "," minimum-separation
        "," max-align-turn
        "," max-cohere-turn
...
to-report cn
  ...
  ask turtles [
    if any? flockmates
    [ find-nearest-neighbor
         set n-n nearest-neighbor
   ]
  ]
  report n-n
end

to-report get-flockmates
  let my-flockmates []
  ask turtles [
    set my-flockmates other turtles in-radius vision ]
  report my-flockmates
end

to-report count-neighbors
  let n-neighbors []
  ask turtles [
    set n-neighbors count other turtles in-radius vision]
  report n-neighbors
  print n-neighbors
end
...
to-report turtle-data
  let data-list []
  ask turtles [
    let turtle-id who
    let neighbors-count count other turtles in-radius vision
    let turtle-info (list turtle-id neighbors-count)
    set data-list lput turtle-info data-list
  ]
  report data-list
end
